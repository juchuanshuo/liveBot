...
import cn.jcsmallming.entry.dynamiccontent.DynamicDetailApiBody;
import cn.jcsmallming.entry.dynamiccontent.DynamicDetailContentCard;
import cn.jcsmallming.entry.dynamiccontent.Pictures;
        ...
        if(type == 1){
            ...
            if(orig_type==8){
                ...
                chain = new MessageChainBuilder()
                        ...
                        .append(new PlainText("\n视频简介：" + cardBody.getOrigin().getDesc()))
                        .append(new PlainText("\n视频地址：" + cardBody.getOrigin().getShort_link()))
                        .append(new PlainText("\n\n发布时间：" + dateString))
                        .build();
//                qqBot.getBot().getGroup(UserInfo.group).sendMessage(chain);
                ...
                //转发带图片动态
                chain = new MessageChainBuilder()
                        .append(new PlainText("你老婆【绯赤艾莉欧Official】刚刚转发了带图片的动态，由于我懒得解析多一层内容所以自己去看吧"))
                        .append(new PlainText(cardBody.getItem().getContent()))
                        .append(new PlainText("\n\n发布时间：" + dateString))
                        .build();
                qqBot.getBot().getGroup(UserInfo.group).sendMessage(chain);
                ...
                chain = new MessageChainBuilder()
                        ...
                        .append(new PlainText(cardBody.getItem().getContent()))
                        .append(new PlainText("\n直播间链接："))
                        .append(new PlainText(cardBody.getOrigin().getSlide_link()))
                        .append(new PlainText("\n\n发布时间：" + dateString))
                        .build();
                qqBot.getBot().getGroup(UserInfo.group).sendMessage(chain);
            }else if(orig_type==4){
